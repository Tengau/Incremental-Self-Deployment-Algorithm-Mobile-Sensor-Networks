...
def plot_map(config, odometry, laser, map, i):
    ...
    if (
        i != len(odometry) - 1
    ):  # If it's not the last iteration, plot the robot and Lidar.
        # Create a wedge (half-circle) representing the LIDAR's field of view
        pos = (
            odometry[i] * resolution
        )  # Compute the robot's position with the resolution taken into account.
        ...
        rob = patches.Circle(
            (pos[0], pos[1]), resolution, fc=config["plot"]["robot_color"]
        )
