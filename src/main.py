...
def main():
    ...
    # algorithms = ["boundary_coverage_alg"]
    ...
    for idx, mode in enumerate(algorithms):
        occ_sim = OccupancyGridSimulator(
            file_name, starting_pose=np.array([200.0, 500.0])
        )
        ...
        for timestep in tqdm(range(t_total), desc="Simulating", unit="step"):
            ...
            if mode == "boundary_alg":
                ...
                new_pose = find_max_coverage_max_boundary_location(
                    new_map, occ_sim.robot_pos_mask, occ_sim.circle_arr, 50, 1
                )
    ...
    np.savetxt(
        f"../data/output/coverages_{t_total}_nodes_5m_sensor_range_hospital.csv",
        coverages,
        delimiter=",",
        fmt="%.3f",
    ...
    np.savetxt(
        f"../data/output/runtimes_{t_total}_nodes_5m_sensor_range_hospital.csv",
        runtimes,
        delimiter=",",
        fmt="%.3f",
    )  # You can adjust `fmt` as needed
