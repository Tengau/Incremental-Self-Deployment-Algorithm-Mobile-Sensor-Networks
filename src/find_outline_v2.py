...
class Occu:
    ...
    def draw_map_with_features(self, output_file=None, include_outline_points=True):
        ...
        if include_outline_points:
            random_outline_locations = self.find_random_outline_location(
                num_locations=3
            )
        ...
        for point in random_locations:
            cv2.circle(
                color_map, (point[1], point[0]), 10, (255, 0, 0), -1
            )  # BGR format: Blue
        ...
        if include_outline_points:
            for point in random_outline_locations:
                cv2.circle(
                    color_map, (point[1], point[0]), 10, (0, 255, 0), -1
                )  # BGR format: Green
        ...
        plt.title(
            "Map with Free Locations (Blue), Outline (Red), and Outline Locations (Green)"
        )
        plt.show()
    ...
    def plot_map_with_features(
        self, random_points=None, outline_points=None, outline_locations=None
    ):
        """
        ...
        if outline_locations is not None:
            plt.scatter(
                ...
                c="green",
                s=50,
                label="Random Outline Locations",
            )
...
def main():
    ...
    occ_sim = Occu(file_name, starting_pose=np.array([800.0, 700]))
    ...
    print(
        "Drawing map with random free locations, outline, and random outline locations..."
    )
